...
void mainGLApp::frameInit()
{
    ...
    windowTitle += " - SDL";
    mainSDLWwnd = SDL_CreateWindow(getWindowTitle(), SDL_WINDOWPOS_CENTERED,
        SDL_WINDOWPOS_CENTERED, GetWidth(), GetHeight(),
...
void mainGLApp::frameInit()
{
    ...
    windowTitle += " - GLFW";
    setGLFWWnd(glfwCreateWindow(
        GetWidth(), GetHeight(), getWindowTitle(), NULL, NULL));
    if (!getGLFWWnd()) {
        glfwTerminate();
        exit(EXIT_FAILURE);
    }
    ...
    while (!done) {
        ...
        while (SDL_PollEvent(&event)) {
            ...
            if (event.type == SDL_MOUSEBUTTONDOWN) {
                theWnd->onMouseButton(event.button.button, SDL_MOUSEBUTTONDOWN,
                    event.button.x, event.button.y);
            }
            if (event.type == SDL_MOUSEBUTTONUP) {
                theWnd->onMouseButton(event.button.button, SDL_MOUSEBUTTONUP,
                    event.button.x, event.button.y);
            }
