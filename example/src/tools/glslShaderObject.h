...
class ShaderObject
{
  public:
    ...
    {
        if (getStatus() == ShaderObject::attached ||
            getStatus() == ShaderObject::wantDetach) {
...
#ifndef NDEBUG
void
CheckErrorsGL(const char* location = NULL, std::ostream& ostr = std::cerr);
#else
inline void
CheckErrorsGL(const char* location = NULL, std::ostream& ostr = std::cerr)
{}
...
void
getFileContents(const char* fileName, string& str);

void
checkDeletedShader(GLuint shader);
void
getCompilerLog(GLuint handle, GLint blen, bool isShader);

int
CheckGLError(const char* file, int line);
void
CheckShaderError(GLuint hProg);
