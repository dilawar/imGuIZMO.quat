...
enum { evLeftButton, evRightButton, evMiddleButton };
...
public:
    ...
    //
    virtual void mouse(
        vgButtons button, vgModifiers mod, bool pressed, T x, T y)
    {
        if ((button == tbControlButton) && pressed
            && (tbControlModifiers ? tbControlModifiers & mod
    ...
    void updateGizmo()
    {
        ...
        T AdotB = dot(a, b);
        T angle = acos(AdotB > T(1)
                ? T(1)
                : (AdotB < -T(1) ? -T(1)
                                 : AdotB)); // clamp need!!! corss float is
                                            // approximate to FLT_EPSILON

        qtStep = normalize(
            angleAxis(angle * tbScale * fpsRatio, axis * rotationVector));
        qtIdle
            = normalize(angleAxis(angle * tbScale * fpsRatio * qIdleSpeedRatio,
                axis * rotationVector));
        qtV = qtStep * qtV;
    }
...
protected:
    ...
    // UI commands that this virtualGizmo responds to (defaults to left mouse
    // button with no modifier key)
    vgButtons tbControlButton, tbRotationButton;
    vgModifiers tbControlModifiers, xRotationModifier, yRotationModifier,
        zRotationModifier;
...
public:
    ...
    void mouse(vgButtons button, vgModifiers mod, bool pressed, T x, T y)
    {
        ...
        if (button == dollyControlButton && pressed
            && (dollyControlModifiers ? dollyControlModifiers & mod
                                      : dollyControlModifiers == mod)) {
            dollyActive = true;
        ...
        if (button == panControlButton && pressed
            && (panControlModifiers ? panControlModifiers & mod
                                    : panControlModifiers == mod)) {
            panActive = true;
            this->activateMouse(x, y);
    ...
    {
        povPanDollyFactor = z;
        dolly.z += y * dollyScale * wheelScale
    ...
    {
        povPanDollyFactor = z;
        if (this->tbActive || dollyActive || panActive)
            VGIZMO_BASE_CLASS::motion(x, y);
    }
    ...
    tMat4 getTransform()
    {
        ...
        // create the translations to move the center of rotation to the origin
        // and back
        trans = translate(tMat4(T(1)), this->rotationCenter);
        invTrans = translate(tMat4(T(1)), -this->rotationCenter);
...
private:
    ...
    // Variable used to determine if the manipulator is presently tracking the
    // mouse
    bool dollyActive;
    bool panActive;
    ...
    T povPanDollyFactor = T(0); // internal use, maintain memory of current
                                // distance (pan/zoom speed by distance)
};
