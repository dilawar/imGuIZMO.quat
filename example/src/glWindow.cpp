...
#ifdef GLAPP_USE_VIRTUALGIZMO // Enable it in glWindows.h or with compiler
                              // define
...
void setView()
{
    ...
    theWnd->projectionMatrix = perspective(radians(30.0f),
        (float)theApp->GetWidth() / (float)theApp->GetHeight(), 0.0f, 30.0f);
...
void setViewOrtho()
{
    ...
    theWnd->mvpMatrix
        = theWnd->projectionMatrix * theWnd->viewMatrix * theWnd->modelMatrix;
}
...
void glWindow::onInit()
{
...
#ifdef GLAPP_USE_VIRTUALGIZMO
#ifdef GLAPP_USE_SDL
    getGizmo().setGizmoRotControl(
        (vgButtons)SDL_BUTTON_LEFT, (vgModifiers)0 /* evNoModifier */);

    getGizmo().setGizmoRotXControl(
        (vgButtons)SDL_BUTTON_LEFT, (vgModifiers)KMOD_SHIFT);
    getGizmo().setGizmoRotYControl(
        (vgButtons)SDL_BUTTON_LEFT, (vgModifiers)KMOD_CTRL);
    getGizmo().setGizmoRotZControl(
        (vgButtons)SDL_BUTTON_LEFT, (vgModifiers)KMOD_ALT);

    getGizmo().setDollyControl((vgButtons)SDL_BUTTON_RIGHT, (vgModifiers)0);
    getGizmo().setPanControl(
        (vgButtons)SDL_BUTTON_RIGHT, (vgModifiers)KMOD_CTRL | KMOD_SHIFT);
#else
    getGizmo().setGizmoRotControl(
        (vgButtons)GLFW_MOUSE_BUTTON_LEFT, (vgModifiers)0 /* evNoModifier */);

    getGizmo().setGizmoRotXControl(
        (vgButtons)GLFW_MOUSE_BUTTON_LEFT, (vgModifiers)GLFW_MOD_SHIFT);
    getGizmo().setGizmoRotYControl(
        (vgButtons)GLFW_MOUSE_BUTTON_LEFT, (vgModifiers)GLFW_MOD_CONTROL);
    getGizmo().setGizmoRotZControl((vgButtons)GLFW_MOUSE_BUTTON_LEFT,
        (vgModifiers)GLFW_MOD_ALT | GLFW_MOD_SUPER);

    getGizmo().setDollyControl(
        (vgButtons)GLFW_MOUSE_BUTTON_RIGHT, (vgModifiers)0);
    getGizmo().setPanControl((vgButtons)GLFW_MOUSE_BUTTON_RIGHT,
        (vgModifiers)GLFW_MOD_CONTROL | GLFW_MOD_SHIFT);
#endif
    // getGizmo().setPanControls(  (vgButtons) GLFW_MOUSE_BUTTON_RIGHT,
    // (vgModifiers) GLFW_MOD_SHIFT);
    ...
    float scale = 1.f
        / (theApp->GetWidth() > theApp->GetHeight() ? theApp->GetHeight()
                                                    : theApp->GetWidth());
...
//
/////////////////////////////////////////////////
void glWindow::onExit() { delete qjSet; }
